...
class GameController: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        dd.game = self
        dd.titlelabel.backgroundColor = UIColor(red: 0, green: 0, blue: 200/255, alpha: 1)
        dd.titlelabel.text = newsarray[indexPath.item].title
        dd.deslabel.text = newsarray[indexPath.item].description
        guard let news = newsarray[indexPath.item].urlimage as? AnyObject else { dd.imageview.backgroundColor = UIColor.red
            return dd}
        ...
        dd.authorlabel.text = "source: Financial Time"
    ...
    func callapi(){
        let ss = "https://newsapi.org/v1/articles?source=financial-times&sortBy=latest&apiKey=cd9a19968a734d1cb7213cc94fcee0f4"
            ...
            var task = session.dataTask(with: request as URLRequest, completionHandler: {data, response, error -> Void in
                ...
                do{
                    ...
                    for i in jd
                    {
                        ...
                        self.newsarray.append(news(author: author, description: "", title: title,url: url, urlimage: urlimage, date: date))
